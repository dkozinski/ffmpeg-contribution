...
PATH="$HOME/bin:$PATH" PKG_CONFIG_PATH="$HOME/ffapv_build/lib/pkgconfig" ./configure --prefix="$HOME/ffapv_build" --extra-libs="-lpthread -lm" --enable-ffplay --enable-shared --disable-static --enable-liboapv --enable-gpl --enable-libx264 && make -j $(nproc) && make install
